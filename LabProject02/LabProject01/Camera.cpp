...
void CCamera::RegenerateViewMatrix(const XMFLOAT3& playerLookDirection, const XMFLOAT3& playerUpDirection)
{
	...
	auto originPos =  Vector3::Subtract(m_xmf3Position, GetOffset());
	m_xmf4x4View._41 = -Vector3::DotProduct(originPos, m_xmf3Right);
	m_xmf4x4View._42 = -Vector3::DotProduct(originPos, m_xmf3Up);
	m_xmf4x4View._43 = -Vector3::DotProduct(originPos, m_xmf3Look);
...
void CCamera::RegenerateViewMatrix() {
	...
	//카메라의 z측과 x축에 수직인 벡터를 y축으로 설저어한다.
...
void CSpaceShipCamera::Rotate(float x, float y, float z) {
	...
	if (m_pPlayer && (x != 0.0f)) {
		...
		//플레이어의 로컬 X축에 대한 X각도의 회전 행렬을 계산하낟.

		XMFLOAT3 xmf3Right = m_pPlayer->GetRightVector();
		XMMATRIX xmmtRotate = XMMatrixRotationAxis(XMLoadFloat3(&xmf3Right), XMConvertToRadians(x));
		//카메라의 로컬 x ,y  z축을 회전한다.

		m_xmf3Right = Vector3::TransformNormal(m_xmf3Right, xmmtRotate);
		m_xmf3Up = Vector3::TransformNormal(m_xmf3Up, xmmtRotate);
		m_xmf3Look = Vector3::TransformNormal(m_xmf3Look, xmmtRotate);
		//카메라의 위치벡터에서 플레이어의 위치벡터(플레이어를 기준으로 한)를 회전한다.

		m_xmf3Position = Vector3::Subtract(m_xmf3Position, m_pPlayer->GetPosition());
		//플레이어의 위치를 중심으로 카메라의 위치 벡터를 회전한다.
		m_xmf3Position = Vector3::TransformCoord(m_xmf3Position, xmmtRotate);
...
void CThirdPersonCamera::SetLookAt(XMFLOAT3& xmf3LookAt) {
	...
	//카메라 변환 행렬에서 카메라의 x-축 t축 z축을 구한다.
