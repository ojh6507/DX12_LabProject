...
void CScene::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	std::uniform_real_distribution<float> distribution(-1000, 100);
	...
	for (int i = 0; i < m_ppGameObjects.size(); i++) {
		...
		for (int j = 0; j < ppBullets2.size(); j++) {
			...
			if (ppBullets2[j]->m_bActive && m_pPlayer->xmBoundingBox.Intersects(ppBullets2[j]->xmBoundingBox)) {
				if (m_pPlayer->m_bBarrier) m_pPlayer->m_bBarrier = false;
				else m_pPlayer->m_bBlowingUp =true;
				ppBullets2[j]->SetActive(false);
			}
		}
		
		for (int j = 0; j < ppBullets.size(); j++) {
			if (ppBullets[j]->m_bActive &&  object->xmBoundingBox.Intersects(ppBullets[j]->xmBoundingBox)) {
				object->m_bBlowingUp =true;
				ppBullets[j]->SetActive(false);
			}
		}
	}
}
